...
export default {
  ...
  data() {
    return {
      counter: {},
      unitText: '',
      countId: 'count' + parseInt(Math.random() * 1000000, 10), // eslint-disable-line prefer-template
    };
  },
  props: {
    mainClass: String,
      ...
      if (val < Math.pow(10, this.unit[0][0])) {
        ...
        for (let i = 1; i < len; i++) {
          if (val >= Math.pow(10, this.unit[i - 1][0]) && val < Math.pow(10, this.unit[i][0])) {
            endVal = parseInt(val / Math.pow(10, this.unit[i - 1][0]), 10);
            unit = this.unit[i - 1][1];
          }
        }
      }
      if (val > Math.pow(10, this.unit[len - 1][0])) {
        endVal = parseInt(val / Math.pow(10, this.unit[len - 1][0]), 10); // eslint-disable-line no-restricted-properties // eslint-disable-line max-len
  ...
  mounted() {
    this.$nextTick(() => {
      setTimeout(() => {
        ...
        counter = new CountUp(this.countId, this.startVal, endVal, this.decimals, this.duration, {
